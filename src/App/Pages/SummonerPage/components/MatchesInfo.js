  ...
  return (
    <div className="matchesinfo">
      <div className="item">
        ...
        <br />
          {`${soloMatches === 'undefined' && !isNaN(flexWinRate) ? '0' : isNaN(soloMatches) ? '0' : soloMatches} matches`}</p>
      </div>
      <div className="item">
        <h4>TFT</h4><p>
          <span>{`${isNaN(tftWinRate) ? 0 : tftWinRate}%`}</span> won in
        <br />
          {`${tftMatches === 'undefined' && !isNaN(flexWinRate) ? '0' : isNaN(tftMatches) ? '0' : tftMatches} matches`}</p>
      </div>
      <div className="item">
        <h4>Flex</h4><p>
          <span>{`${isNaN(flexWinRate) ? 0 : flexWinRate}%`}</span> won in
        <br />
          {`${flexMatches === 'undefined' && !isNaN(flexWinRate) ? '0' : isNaN(flexWinRate) ? '0' : flexWinRate} matches`}</p>
      </div>
    </div>
  );
}
